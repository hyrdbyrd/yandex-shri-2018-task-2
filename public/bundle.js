...
(function () {
    window.addEventListener('load', function (e) {
        ...
        function handler(event) {
            ...
            var wrapperOptions = div.cloneNode(false);
            ...
            if (state) {
                var fullToggle = function fullToggle(event) {
                    toggle();
                    removewrapper();
                };
                ...
                wrapperOptions.appendChild(close);
                wrapperOptions.appendChild(apply);

                wrapper.appendChild(element);
                wrapper.appendChild(wrapperOptions);

                regulator();

                if (paste) {
                    paste.insertAdjacentElement(flag, wrapper);
                } else {
            ...
            function regulator() {
                ...
                if (classes.contains('device-info_temperature_active')) {
                    var intro = element.querySelector('.device-intro-info');

                    var t = div.cloneNode(false);
            ...
            function removewrapper() {
                setTimeout(function () {
                    wrapper.removeChild(wrapperOptions);
                    var paste = wrapper.previousElementSibling;
                    var flag = 'afterEnd';
                    var parent = wrapper.parentElement;
                    if (!paste) {
                        flag = 'beforeBegin';
                        paste = wrapper.nextElementSibling;
                    }
                    ...
                    var intro = element.querySelector('.device-intro');
                    var t = intro.querySelector('.temperature-info');
                    if (t) intro.removeChild(t);
                }, 200);
            }
            ...
            function fullRebase() {
                ...
                if (state) {
                    rebase(wrapper, 'wrapper', 'wrapper_active', 0, false);
                }
            }
        }
...
(function () {
    window.addEventListener('load', function (e) {
        ...
        optionsList.forEach(function (block) {
            ...
            var end = list.offsetWidth;
            var diff = 2;

            function scroll(dir) {
                var summ = 0;
                var scrolling = setInterval(function () {
                    if (summ >= end || list.scrollLeft + list.offsetWidth >= list.scrollWidth && dir > 0 || !list.scrollLeft && dir < 0) clearInterval(scrolling);
